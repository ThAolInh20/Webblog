<template>
    ...
    export default {
      ...
      methods: {
        async fetchUser() {
          try {
            const response = await axios.get(`http://localhost:8000/api/users/${this.id}`);
            this.user = response.data.data; // Gán dữ liệu từ API
          } catch (error) {
        ...
        async updateUser() {
          try {
            await axios.put(`http://localhost:8000/api/users/${this.id}`, this.user);
